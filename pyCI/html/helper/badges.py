...
def image(route):
    slug = route[2]
    db = DB('/etc/pyci.db')
    status = db.project_status_by_slug(slug)
    status_id = UNKNOWN

    if status == Repository.SUCCESS:
        status_id = PASSING
    elif status == Repository.FAILED:
        status_id = FAILING

    return 'mime::svg', badge(status_id)
